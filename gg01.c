...
long endofsentence,
     ...
     syntaxerrors,
     line,
     init_sum = 224,		       /* current version number */
     sum = 0,
     nrterms,
     nrntmems,
     lastsettype,
     ...
     name_clashes,
     optimize_flag,
     no_recursion_elm_flag = 1,
     all_external_flag,
     start_predicate_flag = 0;
...
static char *getglammarenv (char *envvar);
static char *glm_incdir ();
static char *glm_libdir ();
static long have_prog ();
static long newcc (long cc);
static long no_object (char *repr);
static void c_compiler ();
static void print_usage ();
static void read_glm_option ();
static void write_part_file ();
...
{
  long input_file_set = false;
  long output_file_set = false;
  ...
  for (arguments += arg_count - 1; arg_zero != *arguments; arguments -= 1)
  {
	...
	switch (*(++*arguments))
	{
	  ...
	  case 'r':
	    ...
	    break;
	  case 'O':
	    (void) sprintf (tempbuf, " -%c %s ", *(*arguments + 1),
		*arguments + 2);
	  ...
	  case 'V':
	    ...
	    break;
	  case 'v':
	    fprintf (stderr, "Glammar (c) %s (c compiler = \"%s %s\")\n", VERSION, CC,CCFLAGS);
	  case 'U':
	    meta_uniq_flag = true;
	    break;
	  case 'M':
	  ...
	  case 'm':
	    memo_flag = true;
	    sscanf (*arguments + 1, "%ld", &runtime_input_size);
  ...
  {
    (void) sprintf (indexfilename, "%.200s.x", thispart);
  ...
  {
    (void) sprintf (tagindexfilename, "%.200s.gtags", thispart);
  ...
  {
    fprintf (stderr,
	"Glammar fatal msg: not enough heap space available\n");
    exit (1);
  }
  if (verbose_flag)
...
static void read_glm_option () 
{
  ...
  switch (*args)
  {
    ...
    case 'm':
      memo_flag = true;
      sscanf (args + 1, "%ld", &runtime_input_size);
      if (runtime_input_size < 1)
	runtime_input_size = 2;
      runtime_input_size <<= 10;
      break;
...
static void c_compiler () 
{
...
#else
  {
    ...
    *(p - 1) = 'o';
    *p++ = ' ';
    (void) sprintf (commandbuffer,
	"link /library=glammar:glam /executable=%s %s",
	ex_filename, ofile);
  }
#endif
