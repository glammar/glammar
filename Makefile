...
SYSLINTFLAGS = -hpac
SYSCFLAGS =  -Xa -xs -xstrconst -mr -xcg92 
SYSCPROFFLAGS =
SYSCDBUGFLAGS =
SYSPFLAGS =
...
dg-ux DG-UX aviion AVIION:
	@echo SYS=$(SYS)
	make allsystems  CC="gcc -pipe " MPE="ge40.c"\
            VERSION="`cat .version` `date`"\
            COPT=-O
...
dec DEC:
	@echo SYS=$(SYS)
	make allsystems NOMP=-DNOMP LONGJ=-J\
            VERSION="`cat .version` `date`"
...
dec_alpha DEC_ALPHA:
	@echo SYS=$(SYS)
	make allsystems ADDRESSING=-DEIGHT_BYTE_ADDR\
            VERSION="`cat .version` `date`"

orion ORION:
	@echo SYS=$(SYS)
	make first RMOBJECT=-DRMOBJECT COPT=-O  WARN=-w\
            VERSION="`cat .version` `date`"\
            NOMP=-DNOMP 
	make second
